// IMPLEMENTACIÓN DE FUNCIONALIDADES COMUNES RELACIONADAS CON UN CLIENTE DE SOCKETS.
// INCLUYE IMPLEMENTACIÓN DE FUNCIONES QUE CREAN Y CONECTAN UN SOCKET
...
int create_socket_cln_by_addr(unsigned int ip, unsigned short port) {
    ...
    return create_socket_cln((struct sockaddr *)&addr, sizeof(addr));
}
