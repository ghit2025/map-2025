...
int main(int argc, char *argv[])
{
    ...
    if ((s_srv = create_socket_srv(0, &port)) < 0)
        return 1;
    ...
    while (1)
    {
        ...
        {
            char buf[1024];
            if (recv_string(s_conec, buf, sizeof(buf)) < 0)
            {
                perror("error en recv");
                break;
            }
        ...
        while (1)
        {
            ...
            if (pid == 0)
            {
                ...
                char out_file[1024];
                size_t need = strlen(output) + 1 + strlen(base) + 1 + 5 + 1; // dir + '/' + base + '-' + 00000 + '\0'
                if (need > sizeof(out_file))
                {
